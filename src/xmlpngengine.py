...
def calculate_final_size(imdict, imlist, num_cols, clip, reuse):
    ...
    for frame, _ in imlist:
        try:
            ...
            if not clip:
                widths.append(im.width + 2*DEFAULT_PADDING) # padding on each side
                heights.append(im.height + 2*DEFAULT_PADDING)
            else:
                box = im.getbbox()
                widths.append(box[2] - box[0] + 2*DEFAULT_PADDING)
                heights.append(box[3] - box[1] + 2*DEFAULT_PADDING)
            im.close()
    ...
    for impath, coords_dict in imdict.items():
        ...
        for (x, y, w, h), poselist in coords_dict.items():
            if reuse >= 1:
                if not clip:
                    widths.append(w + 2*DEFAULT_PADDING)
                    heights.append(h + 2*DEFAULT_PADDING)
                else:

...
# trying out a new implementation
...
def _appendIconToGrid(icongrid_path, iconpaths, iconsize=150):
...
def appendIconToIconGrid(icongrid_path, iconpaths, iconsize=150): # savedir,
    ...
    for iconpath in iconpaths:
        ...
        if lastrow_y >= icongrid.height:
            # we probably need to add another row to insert new image
            icongrid = _icongrid_add_row(icongrid)
            # clean_up(icongrid, iconimg)
            # return 1, new_index, None, exception_msg # 1, None, None, None
        row_index = lastrow_y // iconsize

        _last_row_img = icongrid.crop((0, row_index*iconsize, icongrid.width, row_index*iconsize + iconsize))
        box = _last_row_img.getbbox()
        _last_row_img.close()
        ...
        if box:
            ...
            if row_index >= max_row - 1 and col_index >= max_col - 1:
                # we need to expand the icon-grid (add a new row)
                icongrid = _icongrid_add_row(icongrid)
